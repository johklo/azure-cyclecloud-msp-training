...
이 문서는 **Azure CycleCloud 8.8.1+** 환경에서 Azure Managed Prometheus 및 Managed Grafana로 GPU/CPU 자원 사용률을 모니터링하는 절차를 다룹니다.
...
3. **Managed Grafana**: Azure Managed Grafana 대시보드에서 GPU 사용률, 메모리 점유율, 온도, 전력 소비량 시각화 및 알림 설정
...
CycleCloud 4.0.3+ 는 **Monitoring 탭** 활성화 시 Node Exporter, NVIDIA **DCGM** Exporter(GPU 노드), AzSlurm Exporter(스케줄러)를 자동 설치합니다. 노드에서 `curl` 로 지표 노출을 확인합니다.
...
특정 Azure 리전(예: Mexico Region, Korea South 등)에서는 **Azure Managed Grafana가 지원되지 않거나 수집 서비스 리전 제약**이 있을 수 있습니다.
