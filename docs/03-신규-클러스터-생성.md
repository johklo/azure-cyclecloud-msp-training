...
> ⚠️ **사전 권한 부여 필수**: **Validate Credential** 전에, 서버 VM(`cc-server`)의 **시스템 할당 관리 ID**에 아래 두 역할을 부여해야 합니다.
> - **Contributor** — **구독** 범위(Subscriptions → 대상 구독 → Access control(IAM))
> - **Storage Blob Data Contributor** — **Locker 스토리지 계정** 범위(Storage account → Access control(IAM))
>
> 할당 직후에는 반영까지 수 분이 걸릴 수 있어 Validate가 실패하면 잠시 후 재시도합니다. (최소 권한이 필요하면 Contributor를 대상 RG 범위로 좁히거나 커스텀 RBAC 역할로 대체 가능)
>
...
- **ManagedIdentity**: **`locker-mi`** 선택 ⭐ — **이 필드가 노드에 `locker-mi`(Blob Data Reader)를 연결**하는 지점입니다. 지정하지 않으면 노드가 Locker 접근 권한을 얻지 못해 부팅/프로비저닝이 실패할 수 있습니다.
