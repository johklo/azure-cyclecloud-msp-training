...
## 3.0 선제 요구 사항 (Prerequisites) — 먼저 확인

클러스터 생성에 앞서 **네트워크 도달성**이 확보되지 않으면, 서버가 프로젝트를 Locker에 스테이징하거나 노드가 부팅에 필요한 파일을 내려받지 못해 **403/타임아웃으로 클러스터 생성이 실패**합니다. 아래 두 경로를 반드시 먼저 점검하세요.

CycleCloud의 데이터 흐름은 **서로 다른 3개 경로**로 나뉩니다. 각 경로가 열려 있어야 합니다.

```
① 인터넷 → 서버 (아웃바운드 egress)   : 프로젝트/패키지/제어플레인 API
② 서버 → Locker(스토리지)            : 프로젝트 blob 업로드 (사설/PE)
③ Locker(스토리지) → 노드            : 부팅 시 프로젝트 다운로드 (사설/PE)
```

> ⚠️ **스토리지 계정은 인터넷에서 직접 무언가를 가져오지 않습니다.** 서버가 중개자입니다: `인터넷 → 서버(egress) → Locker(사설) → 노드(사설)`. 따라서 ①(서버의 아웃바운드)과 ②③(사설 스토리지 경로)은 **별개로** 점검해야 합니다.

### 3.0.1 서버 아웃바운드 엔드포인트 (① 경로 · 잠긴 망)

서버가 **사설 IP이고 인터넷이 차단된 환경**이라도, **모든 아웃바운드를 완전 차단하면 CycleCloud는 동작하지 않습니다.** 아래 **Azure 제어플레인 엔드포인트**는 인증·VM 생성에 필수이므로, NSG **Service Tag**로 허용해야 합니다(일반 인터넷 개방이 아니라 Microsoft 백본에 한정됨).

| 용도 | 엔드포인트(FQDN) | 허용 방법(NSG Service Tag) |
|------|------------------|---------------------------|
| VM 생성/관리 (ARM) | `management.azure.com` | `AzureResourceManager` |
| 인증 (Entra ID) | `login.microsoftonline.com` | `AzureActiveDirectory` |
| Locker 스토리지 | `*.blob.core.windows.net` | `Storage` (또는 **Private Endpoint**, 3.0.2 권장) |
...
> **핵심**: `management.azure.com`, `login.microsoftonline.com`은 **Private Endpoint가 없습니다.** 반드시 아웃바운드(Service Tag)로 허용해야 하며, 이것까지 막으면 어떤 방법으로도 클러스터를 만들 수 없습니다. "제로 egress"는 불가능하고, "**필수 Service Tag만 허용 + 나머지 인터넷 차단**"이 올바른 잠긴 망 구성입니다.
>
> **소프트웨어·프로젝트 패키지**(github, `packages.microsoft.com`, pypi 등)는 제어플레인과 별개입니다. 완전 잠긴 망에서는 ⓐ **HTTPS 프록시**로 특정 FQDN만 통과시키거나, ⓑ **오프라인/에어갭**(프로젝트를 Locker에 사전 적재 · 패키지는 내부 미러 · 소프트웨어는 커스텀 이미지에 사전 설치)으로 처리합니다.

**공식 문서**
- 잠긴 망에서 실행(필수 아웃바운드 URL): <https://learn.microsoft.com/azure/cyclecloud/how-to/running-in-locked-down-network?view=cyclecloud-8>
- 웹 프록시 사용: <https://learn.microsoft.com/azure/cyclecloud/how-to/running-behind-proxy?view=cyclecloud-8>
- 보안 모범 사례: <https://learn.microsoft.com/azure/cyclecloud/concepts/security-best-practices?view=cyclecloud-8>
- NSG Service Tag 목록: <https://learn.microsoft.com/azure/virtual-network/service-tags-overview>

### 3.0.2 Locker 스토리지 도달성 (②③ 경로 · 사설)

Locker로 쓰는 스토리지 계정이 **공용 접근 차단(`publicNetworkAccess=Disabled`)** 상태이고 **Private Endpoint가 없으면**, 서버·노드가 스토리지에 아예 도달하지 못해 스테이징 단계에서 **`HttpStatus-403`** 으로 실패합니다(자세한 진단은 [`11-트러블슈팅-로그.md`](11-트러블슈팅-로그.md) §11.9 참고). 조직 정책이 공용 접근을 강제로 막는 경우가 많으므로, **Private Endpoint** 구성이 정석입니다.

**점검 항목**
1. **RBAC**: 서버 시스템 MI = **Storage Blob Data Contributor**, `locker-mi` = **Storage Blob Data Reader**(§3.1 참고)가 스토리지 계정 범위에 부여됐는가.
2. **컨테이너 존재**: Locker 컨테이너(예: `cyclecloud`)가 **미리 생성**되어 있는가.
3. **네트워크 도달성** (택1):
   - **(권장) Private Endpoint**: 스토리지 blob 하위리소스용 PE를 **노드/서버가 속한 VNet**에 생성 → `privatelink.blob.core.windows.net` **Private DNS zone**을 그 VNet에 링크 → **DNS zone group**으로 A레코드 자동 등록. 이렇게 하면 서버·노드가 스토리지를 **사설 IP로 해석·접근**하며, 공용 접근은 Disabled로 유지해도 됩니다.
   - **(대안) Service Endpoint / 방화벽 규칙**: 스토리지 계정의 네트워킹에서 해당 VNet/서브넷을 허용 목록에 추가.

> 개념 근거(서브넷·Private Endpoint·Private DNS·MI)는 [`01-환경-개요.md`](01-환경-개요.md) §1.6을 참고하세요.
