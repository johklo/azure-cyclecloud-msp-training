...
| **BootDiskSize** | 노드 OS(부팅) 디스크 크기(GB). `0` = 실제 64GB(→ [15. 디스크 사이즈 변경](15-디스크-사이즈-변경.md)). | |
...
- **Cloud-init**: 노드 부팅 시 실행할 스크립트 지정 탭. 변경이 없으면 그대로 진행.
  > ⚠️ 운영 중 cloud-init 수정은 노드 재기동을 유발할 수 있습니다(→ [4장](04-노드-증감설-사이즈변경.md)). 노드 커스터마이징은 **cluster-init** 사용을 권장합니다(→ [2장](02-cluster-init-및-커스텀-스크립트.md)).
