...
이 문서는 **Azure CycleCloud 클러스터 전반**(스케줄러 + 계산 노드)을 **Prometheus + Grafana** 로 모니터링하는 방법을 정리합니다. GPU 지표에 특화된 내용은 [10. GPU 모니터링 구축](10-GPU-모니터링-구축.md)을 함께 참고하세요.
...
CycleCloud 모니터링의 표준 데이터 파이프라인은 다음과 같습니다.
...
> 즉, **노드 단의 Prometheus는 self-hosted**, **중앙 저장·시각화는 Managed** 인 하이브리드 구조가 기본값입니다.
...
```
> `deploy.sh` 는 Azure Monitor Workspace(Prometheus)와 Azure Managed Grafana, 사전 제작 대시보드를 생성합니다.

---

## A.4 2단계: Managed Identity에 Publisher 권한 부여

지표를 Azure Monitor Workspace로 게시하려면 MI에 **Monitoring Metrics Publisher** 역할이 필요합니다. `deploy.sh` 는 MI를 만들지 않으므로, 기존 Locker MI(또는 CCWS의 `ccwLockerManagedIdentity`)를 사용합니다.
...
> 내부적으로 다음 파라미터에 매핑됩니다: `configuration_monitoring_enabled`, `configuration_identity_client_id`, `configuration_ingestion_endpoint`.
...
클러스터 기동 후 배포된 **Azure Managed Grafana** 인스턴스의 Endpoint(포털에서 확인)로 접속 → **Dashboards / Azure CycleCloud** 폴더의 사전 제작 대시보드를 사용합니다.
...
- **대안 1 — Self-hosted Grafana 시각화**: 지표는 그대로 노드 self-hosted Prometheus에서 수집하되, 가까운 리전의 Managed Grafana에 데이터소스로 연결하거나 CycleCloud 서버 VM에 **Grafana를 직접 설치**해 시각화. (리전 간 트래픽 비용 유의)
- **대안 2 — 완전 Self-hosted Prometheus**: Azure Monitor Workspace 대신 중앙에 자체 Prometheus 서버를 두고 노드 Exporter(:9100/:9400/:9101)를 **scrape** 하도록 구성. 폐쇄망·데이터 주권 요건에 적합.
