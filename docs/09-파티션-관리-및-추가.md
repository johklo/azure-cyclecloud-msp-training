...
이 문서는 기존 Slurm 클러스터의 계산 노드를 공유하거나 새로운 물리/논리 파티션을 추가하는 방법, 파티션별 작업 스케줄링 특성, 그리고 **자동 초기화(Reset) 주의사항**을 다룹니다.

---

## 9.1 파티션 설정 수정 (`azure.conf`)

스케줄러 노드의 Slurm 설정 파일(`/etc/slurm/azure.conf`)을 수정하여 새 파티션을 정의할 수 있습니다.
...
노드 1대를 `azslurm suspend`로 제거한 후, 파티션 제출 작업에 따라 노드가 정상적으로 `resume` 되는지 확인합니다.
...
파티션 논리는 분리되었더라도 실제 VM 자원을 공유하므로 물리 Core/Memory 제약을 받습니다.
...
> 스케줄러 노드에서 `/etc/slurm/azure.conf`를 수동 편집하여 파티션을 추가한 경우, 아래 조건에서 **CycleCloud GUI 설정 템플릿으로 원복되어 추가한 파티션이 삭제**됩니다.
>
> 1. 클러스터를 **Terminate 후 Start** 하는 경우
> 2. `azslurm partition` 또는 `azslurm scale` 명령이 수동/자동 실행되는 경우
>
> **대응책**: 영구 파티션 추가 시에는 CycleCloud **클러스터 템플릿(.txt)의 `[[nodearray]]` 및 `[[partition]]` 블록**에 사전 정의해야 합니다.
