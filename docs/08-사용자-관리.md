...
이 문서는 CycleCloud Built-in 사용자 관리, SSH 공개키 등록, 노드 OS 계정 자동동기화 절차를 다룹니다.

---

## 8.1 CycleCloud 사용자 관리 방식 개요

CycleCloud는 아래 사용자 인증/관리 방식을 지원합니다.
...
- **Roles**: 포털 GUI 접속 권한 설정. 노드 SSH 전용 사용자는 역할을 선택하지 않아도 됩니다.
...
3. CycleCloud `jetpack` 데몬이 기동 중인 스케줄러 및 계산 노드 OS에 계정을 생성하고 public key를 `/home/<username>/.ssh/authorized_keys`에 배치합니다.
