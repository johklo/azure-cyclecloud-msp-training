...
## 4.4 KT 운영 지침: GPU VM Scale-in 방지 (`SuspendExcParts`)
...
## 4.6 변경사항을 실행 중인 클러스터에 적용 (`azslurm scale`)
...
> ⚠️ **KeepAlive/좀비 노드**: 포털에서 `KeepAlive=true` 로 두어도 Slurm 내부 상태는 `powered_down` 이 되어 **좀비 노드**(`down~`/`drained~`)가 될 수 있습니다. 축소 방지는 **4.4 `SuspendExcParts`** 또는 **`azslurm keep_alive`** CLI 를 사용하세요. 좀비 노드는 해당 노드에서 `systemctl restart slurmd` 로 복귀하거나 `azslurm suspend` 로 정리합니다.
...
### deallocate가 불가피한 경우 (예: 사이즈 변경 4.5)
