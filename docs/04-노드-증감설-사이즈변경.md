...
Scale-in 제외는 `/etc/slurm/slurm.conf`의 `SuspendExcParts`로 설정합니다. KT RI/GPU 운영 지침은 4.5를 참고하세요.

파티션별 설정: `/etc/slurm/slurm.conf`
...
CycleCloud 노드는 **노드 배열(node array)** 단위로 관리됩니다(예: `scheduler`, `hpc`, `login`).
...
> - 실행 중인 클러스터는 UI 수량 변경만으로 즉시 반영되지 않을 수 있습니다. 스케줄러 `azure.conf` 및 `azslurm` 조정을 병행합니다.

> **실측 사례: 실행 중 클러스터에서 UI 수량 확장/파티션 추가가 반영되지 않음**  
> 실행 중인 클러스터에서 UI로 노드 수를 늘리거나(예: `[1-3]` → `[1-4]`) 새 파티션을 추가하면 `sinfo`는 정상처럼 보여도 실제 노드 할당이 실패할 수 있습니다.
> ```text
> hpc*  up  infinite  2  idle~ cycle-lab-hpc-[1-3]
> Error 'Unknown node name(s): cycle-lab-hpc-4': See the rest in the log file
> ```
> 확장(증설)은 스케줄러에서 `sudo azslurm scale`(4.7) 로 `azure.conf`·`gres.conf` 를 재생성해 반영합니다. 재시작 후 유지하려면 UI 값도 저장합니다.

### 스케줄러에서 수량 조정 (축소만, 재시작 없이)
**감설(축소)** 은 스케줄러의 `azure.conf` 를 직접 편집해 즉시 반영할 수 있습니다. 증설은 위 `azslurm scale`을 사용합니다.
```bash
...
sudo scontrol reconfigure
```
> 수동 편집은 이후 `azslurm scale` 또는 클러스터 재시작 시 **템플릿 기준으로 덮어써집니다**(→ 4.6). 영구 반영은 UI/템플릿 값을 함께 수정하세요.

---

## 4.4 수동 노드 할당 및 회수 (`azslurm`)

스케줄러 노드에서 `azslurm`으로 노드를 프로비저닝(Resume)하거나 회수(Suspend)합니다.
...
> Slurm은 기본적으로 300초(5분) 유휴 노드를 자동 감설합니다. KT 환경의 GPU VM은 **Reserved Instance(RI)** 계약이 적용되어 자동 감설 시 재할당 실패(Capacity 부족) 위험이 큽니다. 포털 `keep_alive` 옵션이 오작동할 수 있으므로 Slurm 설정으로 제외합니다.
...
CycleCloud는 동작 중인 VM 크기를 실시간 변경하지 않습니다. **노드 배열의 SKU 설정 변경 → 기존 노드 Terminate → 새 크기로 재생성** 방식입니다.
...
포털 Edit 또는 템플릿 변경 후 스케줄러 노드에서 `azslurm scale` 을 실행하면 **클러스터 재시작 없이** 변경이 반영됩니다. `azure.conf`·`gres.conf` 를 재생성하고 `slurmctld` 를 재시작하며, 실행 중인 작업에는 영향을 주지 않습니다.
```bash
cyclecloud connect scheduler -c slurm-train
sudo -i
azslurm scale
```
> `azslurm scale` 은 클러스터 **템플릿** 기준으로 `azure.conf` 를 재생성합니다. 스케줄러에서 `azure.conf` 를 수동 편집한 내용은 사라지므로, 영구 반영은 템플릿을 수정해야 합니다.
```bash
...
스케줄러 노드에서 `root` 로 실행하는 자동확장 CLI 입니다.
...
> ⚠️ **KeepAlive/좀비 노드**: 포털에서 `KeepAlive=true` 로 설정해도 Slurm 내부 상태가 `powered_down` 이 되어 **좀비 노드**(`down~`/`drained~`)가 될 수 있습니다. 축소 방지는 **4.5 `SuspendExcParts`** 또는 **`azslurm keep_alive`** CLI 를 사용하세요. 좀비 노드는 해당 노드에서 `systemctl restart slurmd` 로 복귀하거나 `azslurm suspend` 로 정리합니다.

## 4.9 노드 재부팅 SOP (용량/RI 안전 절차)

노드 재부팅은 **"재부팅"과 "재할당(deallocate→start)"을 구분**합니다. 용량 부족 위험은 VM을 **deallocate(할당 해제)** 했다가 다시 켤 때 발생합니다. **할당을 유지한 채 OS만 재부팅**하면 발생하지 않습니다.

> 참고: **Quota ≠ Capacity**. Quota(구독 vCPU 상한)는 증설할 수 있지만, Capacity(리전 물리 GPU 재고)가 없으면 quota가 남아도 재할당이 실패합니다. **RI(Reserved Instance)는 요금 할인일 뿐 물리 용량을 보장하지 않습니다**. 용량 보장은 **On-demand Capacity Reservation** 이 필요합니다.
...
CycleCloud 계산 노드는 **VMSS(가상 머신 확장 집합)** 로 생성됩니다. OS 디스크 용량 변경은 **① 클러스터 전체 반영**과 **② 특정 파티션만 반영** 두 방식이 있습니다. VM SKU 변경 4.6과는 별개입니다.
...
특정 파티션(예: GPU 노드)의 OS 디스크만 바꿀 때는 해당 VMSS 를 직접 수정합니다. 변경은 **새로 뜨는 노드에만** 반영됩니다.
...
> 이 방식은 실행 중인 노드는 유지하고 **자동확장으로 새로 뜨는 노드**에만 큰 디스크가 필요한 경우(예: GPU 워크로드 데이터 캐시)에 사용합니다.
