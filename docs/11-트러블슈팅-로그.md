...
CycleCloud 서버 및 노드 로그 위치, SSH 없이 진단하는 방법, 자주 발생하는 문제와 대응 Triage 표를 정리합니다.
...
Azure RBAC 권한이 있으면 담당자 PC에 SSH 키가 없어도 진단할 수 있습니다.
...
### 2) 계산 노드 내부 에이전트 로그
노드 생성 실패 또는 스크립트 실행 에러는 노드에 SSH 접속해 점검합니다:
```bash
# 노드 직접 접속
...
노드가 Azure에는 할당(Allocation)되었지만 CycleCloud UI 연동 전에 실패하면 `cyclecloud connect` 가 동작하지 않을 수 있습니다. 이때는 **CycleCloud 서버에 내장된 키로 문제 노드에 직접 SSH** 합니다.
```bash
...
> 공인 IP 없는 **Bastion / 서버 점프호스트 접근 모델**은 [1장 §1.6.3 Public IP & 접근 모델](01-환경-개요.md#163-public-ip--접근-모델)을 참고하세요.
...
지원용 로그/구성 데이터를 노드에서 수집합니다. 스케줄러/로그인/실행 노드 어디서나 실행 가능하며, **점검 대상 노드마다** 실행해 생성된 tarball 을 지원 케이스에 첨부합니다.
```bash
...
Slurm `HealthCheckProgram` 이 **healthagent** 를 통해 문제를 감지하면 노드를 **DRAIN** 처리합니다(기본 60초 주기).
```bash
sinfo -R                       # DRAIN/DOWN 노드와 사유 확인
azslurm retry_failed_nodes     # 실패 노드 재시도
# 특정 노드 복구: 해당 노드에서 sudo systemctl restart slurmd
```
> healthagent 는 별도 systemd 서비스이며 CycleCloud UI 에도 상태 오류를 표시합니다.

---

## 11.8 버전 정보 확인 (지원 케이스 필수 정보)

지원 요청 시 CycleCloud/Slurm/Jetpack 버전을 함께 제출합니다.
...
> 버전에 따라 노드 수량 Count 기준이 달라집니다(8.6 이전=코어 / 8.7+=인스턴스, → [4.2](04-노드-증감설-사이즈변경.md)). 지원 문의 전 세 버전을 확인하세요.
...
### 증상
클러스터 생성 또는 노드 프로비저닝 시 다음 오류가 발생하며, 노드가 프로젝트(jetpack)를 내려받지 못합니다.
```
...
- ⚠️ **Azure 포털에서 스토리지 계정만 삭제해도 CycleCloud 내부의 Locker 레코드는 남습니다.** CycleCloud는 스토리지(Azure 리소스)와 **별개로** `Cloud.Locker` 레코드를 datastore에 보관합니다.
...
검증된 방법은 **클러스터에 사용할 Locker를 명시**하는 것입니다. CycleCloud 8.9.1에서 `cyclecloud locker` 에는 **`remove`/`delete` 서브커맨드가 없고**, `cycle_server execute` 로 레코드 삭제 시 **`Cannot delete active resources`** 로 차단됩니다. 아래 **A를 우선** 적용합니다.
...
> credential 하나에 Locker가 여러 개이면 Default 플래그만으로 자동 선택하지 않습니다. 클러스터에 Locker를 지정하면 이 오류가 사라집니다.
...
> ⚠️ **Azure 포털에서 스토리지 계정만 지우면** CycleCloud 레코드는 남아 오류가 지속됩니다. **CycleCloud 측(포털 Settings 또는 credential 재등록)** 에서 정리해야 합니다.
```bash
...
`cyclecloud locker show` 또는 위 SELECT 결과에서 `Status = Failed`, `StatusMessage = Creating container (Http Status 403)` 이면 Locker 생성이 실패한 상태입니다.
